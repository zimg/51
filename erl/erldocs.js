ErlDocs = function () {
    ...
    function searchApps(str)
    {
        ...
        for (var i = 0, count = 0; i < ErlDocs.index_len; i++)
        {
            ...
            {
                var page_name = item[0] == "fun" ? item[4]+".html" : "";
                var url = WEBURL + "mod/" + item[1] + "/" + page_name + "?search=" + str + "&i=" + count;
                html.push('<li class="', item[0], '"><a href="', url, '"><span class="name">', item[2], '</span><br /><span class="sub">', item[3], '</span></a></li>');
                if (count++ > 100)
                {
                    break;
...
ErlDocs.get_jsonp = function (data, callbackfun)
{
    $.ajax({
        type: "get",
        async: false,
        url: "https://" + ACTURL + "/erlshellaction/",
        dataType: "jsonp",
        jsonp: "callback",
        data: data,
        success: callbackfun
...
$(window).load(function () {
    ...
    $('.erlcode').each(function () {
        ...
        $('#' + ebid).click(function () {
            ...
            var cmd_test =  /os:cmd/i;
            var kill_test = /kill/i;
            var erlstr    = erlstr.join(" ");
            if(cmd_test.test(erlstr) && kill_test.test(erlstr))
            {
                var erid = "#erlcode_" + ci + "_ret";
                $(erid).html("");
                $(this).html("同学! 语句很危险！你很优秀！");
                $(this).css("width", "auto");
                $(this).attr("action", "0");
            }else
            {
                var data = { "action": 5, "erl_str": erlstr };
    ...
    if ($('#mod_list_tbody').length > 0) {
        ...
        for (i = 0; i < ErlDocs.index_len; i++)
        {
            ...
            {
                mod_count++;
                _html.push("<tr><td>", mod_count, "</td><td><a href='", WEBURL, "mod/", item[2], "/'>", item[2], "</a></td><td>", item[3], "</td><td id='", item[1], "_count'>0</td></tr>");
            }
            if (item[0] == "fun")
            {
            ...
            for (var i = 0; i < ErlDocs.index_len; i++) {
                ...
                if (item[0] == "fun" && ( mod == "" || mod == item[1] )) {
                    ...
                    if (func_count > start_count && func_count <= end_count) {
                        _html.push("<tr><td>", func_count, "</td><td><a href='", WEBURL, "mod/", item[1], "/'>", item[1], "</a></td><td><a href='", WEBURL, "mod/", item[1], "/", item[4], ".html?search=", item[1], ":'>", item[5], "</a></td><td>", item[3], "</td></tr>");
                    }
                }
            }
    ...
    $("#casual_look").click(function ()
    {
        ...
        var url = WEBURL + "mod/" + item[1] + "/" + func_name + ".html?search=" + item[1] + ":";
        window.location = url;
    });
